...
const IDENTITE = {
  ...
  ProgrammeCategory,
  { organisation: string; titre: string; couleur: string }
>;
    ...
    <div className="mx-auto min-w-[1320px] bg-white px-10 py-9 text-black">
      <div className="mb-5 grid grid-cols-[110px_1fr_110px] items-center">
        ...
        <div className="text-center">
          ...
          <p
            className="mt-1 text-[22px] font-semibold italic text-red-600"
      ...
      <h3 className="mb-7 text-center text-[21px] font-bold">
        {MOIS[mois - 1]} {annee}
      </h3>
      ...
      <table className="w-full table-fixed border-collapse text-[11px]">
        ...
        <thead className="bg-[#fff4b8] font-bold text-[#69571a]">
          <tr>
            ...
            {joursDuMois.map((jour) => (
              <th key={jour} className="border border-gray-400 py-1">
                {JOURS[new Date(annee, mois - 1, jour).getDay()]}
              </th>
            ))}
          </tr>
          <tr>
          ...
          {activites.map((activite, index) => (
            <tr key={activite.id} className="h-[31px]">
                  ...
                  <span className="min-w-0">
                    ...
                    {activite.specifique && (
                      <span
                        ...
                        className="ml-2 rounded bg-amber-100 px-1.5 py-0.5 text-[8px] font-semibold uppercase tracking-wide text-amber-800"
                      >
                        Ce mois
                      </span>
                    )}
                  </span>
                  {(onEditActivity || onMoveActivity || onRemoveActivity) && (
                    <span
                      data-html2canvas-ignore
                      className="flex shrink-0 items-center rounded border border-gray-200 bg-white opacity-30 transition group-hover:opacity-100"
                    >
                      {onMoveActivity && (
                        <>
                          <button
                            onClick={() => onMoveActivity(activite, -1)}
                      ...
                      {onEditActivity && (
                        <button
                          onClick={() => onEditActivity(activite)}
                          className="p-1 text-gray-500 hover:bg-gray-100 hover:text-gray-900"
                          title="Modifier pour ce mois"
                        >
                      ...
                      {onRemoveActivity && (
                        <button
                          onClick={() => onRemoveActivity(activite)}
                          className="p-1 text-gray-500 hover:bg-red-50 hover:text-red-600"
                          title="Retirer de ce mois"
                        >
              ...
              {joursDuMois.map((jour) => {
                ...
                const evenement = activite.evenements.find(
                  (item) => item.jour === jour,
                );
                const contenu = (
                  <>
                    {checked && (
                      <Check
                        className="h-5 w-5 text-[#71104f]"
                        strokeWidth={1.5}
                      />
                    )}
                    {evenement && onDateClick && (
                      <span
                        data-html2canvas-ignore
                        className="absolute right-0.5 top-0.5 h-1.5 w-1.5 rounded-full bg-gray-900"
                      />
                    )}
                  </>
                );
                ...
                return (
                  <td
                    key={jour}
                    className={`border border-gray-400 p-0 ${checked ? "bg-[#efacd9]" : "bg-[#fafafa]"}`}
                  >
                    {onDateClick ? (
                      <button
                        onClick={() => onDateClick(activite.id, jour)}
                        className="relative flex h-[30px] w-full items-center justify-center"
                        aria-label={`${activite.titre}, ${jour} ${MOIS[mois - 1]}`}
                      >
                        {contenu}
                      </button>
                    ) : (
